...
// made to work on Linux and other 64 bit systems by Valentas Kurauskas, released 3 May 2014
...
#include "libhfz_margo.h"
#include "zlib/zlib.h"
#include "zlib/zconf.h"
//#include "zlib/include/zlib.h"
//#include "zlib/include/zconf.h"
//#include <zlib> //why do not work?
//#include <zconf>

// global vars
static int32_t hfzByteOrder = LIBHFZ_BYTEORDER_LITTLEENDIAN; // *NIX developers: change this default
//static long hfzByteOrder = LIBHFZ_BYTEORDER_BIGENDIAN; // *NIX developers: change this default
...
int32_t hfzSave(const char* lpFileName, hfzFormat Format, hfzHeader& fh, float* pData) {
	...
	int32_t nBlocksX = fh.nx / fh.TileSize;
	int32_t nBlocksY = fh.ny / fh.TileSize;
	if(fh.nx%fh.TileSize)	nBlocksX++;
	if(fh.ny%fh.TileSize)	nBlocksY++;
	int32_t BlockCount = nBlocksX * nBlocksY;
	...
	// writing tiles
	int32_t rval = LIBHFZ_STATUS_OK;
	for(int32_t bj = 0; bj<nBlocksY && rval==LIBHFZ_STATUS_OK; bj++) {
	for(int32_t bi = 0; bi<nBlocksX && rval==LIBHFZ_STATUS_OK; bi++) {
...
int32_t hfzLoad(const char* lpFileName, hfzHeader& fh, float *pData) {
	...
	int32_t rval = hfzReadHeader(fs, fh);
	...
	int32_t nx = fh.nx;
	int32_t ny = fh.ny;
	int32_t TileSize = fh.TileSize;

	// determine number of blocks
	int32_t nBlocksX = nx / TileSize;
	int32_t nBlocksY = ny / TileSize;
...
int32_t hfzLoadEx(const char* lpFileName, hfzHeader& fh, float **h_pData) {

	// null header (in case caller forgot to do so)
	hfzHeader_Init(fh, 0, 0, 0, 0, 0, NULL);

	int32_t rval = hfzReadHeader2(lpFileName, fh);
	if(LIBHFZ_STATUS_OK!=rval) {
        return rval;		
	}
	...
	// load 
	rval = hfzLoad(lpFileName, fh, pData);
	if(LIBHFZ_STATUS_OK!=rval) {
		hfzHeader_Reset(fh);
		hfzFree(pData);
		return rval;
...
int32_t hfzWriteHeader(hfzFile* fs, hfzHeader& fh) {
	...
	int32_t rval = hfzHeader_EncodeExtHeaderBuf(fh, &pExtHeaderData);
...
int32_t hfzReadHeader(hfzFile* fs, hfzHeader& fh) {
	...
	uint32_t hl = fh.ExtHeaderLength;
    ...
    if(hl) {
		...
		int32_t rval = hfzHeader_DecodeExtHeaderBuf(fh, pBuf);
...
int32_t hfzReadTile(hfzFile* fs, hfzHeader& fh, uint32_t TileX, uint32_t TileY, float* pMapData) {
	...
	uint32_t i1, i2, j1, j2, i, j;

	char c;
	short s;
	int32_t li;

	uint32_t TileSize = fh.TileSize;
	uint32_t nx = fh.nx;
	uint32_t ny = fh.ny;
...
int32_t hfzWriteTile(hfzFile* fs, hfzHeader& fh, uint32_t TileX, uint32_t TileY, float* pMapData) {
	...
	char c;
	short s;
	uint32_t i1, j1, i2, j2, bnx, bny, zi;
	int32_t FirstVal;
	float VertScale;
	float VertOffset;
	...
	for(j=j1; j<j2; j++) {
		...
		// get first val
		f = pMapData[i1 + j * nx];
		FirstVal = (int32_t)((f-VertOffset)/VertScale);
		int32_t LastVal = FirstVal;
		...
		for(i=i1+1; i<i2; i++) {
			...
			TempInt=(int32_t)((f-VertOffset)/VertScale);
...
//	} else {
		switch(fs->Format) {
		case LIBHFZ_FORMAT_HF2:    return (int32_t)fread(pData, 1, len, (FILE*)fs->pIoStream);
		case LIBHFZ_FORMAT_HF2_GZ: return (int32_t)gzread(fs->pIoStream, pData, len);
//		}
	}
...
//	} else {
		switch(fs->Format) {
		case LIBHFZ_FORMAT_HF2:    return (int32_t)fwrite(pData, 1, len, (FILE*)fs->pIoStream);
		case LIBHFZ_FORMAT_HF2_GZ: return (int32_t)gzwrite(fs->pIoStream, pData, len);
//		}
	}
...
int32_t hfzHeader_Init(hfzHeader &fh, uint32_t nx, uint32_t ny, unsigned short TileSize, float Precis, float HorizScale, uint32_t nExtHeaderBlocks) {
	...
	try {
		...
		if(nExtHeaderBlocks) {
			...
			for(uint32_t i=0; i<nExtHeaderBlocks; i++) {
...
void hfzHeader_Reset(hfzHeader& fh) {
	...
	for(uint32_t i=0; i<fh.nExtHeaderBlocks; i++) {
...
int32_t hfzSetLocalByteOrder(int32_t ByteOrder) {
