    ...
    def data(self, index, role = QtCore.Qt.DisplayRole):
        ...
        if role == QtCore.Qt.DisplayRole:
            ...
            if self.coordDB.column_names[col] in xyz.NUMERIC_FEATURES:
                ...
                return QtCore.QString(str(item))
        return QtCore.QVariant()

    def headerData(self, section, orientation, role):
        if role == QtCore.Qt.DisplayRole:
            if orientation == QtCore.Qt.Horizontal:
                return QtCore.QString(self.coordDB.column_names[section])
        return QtCore.QVariant()
    ...
    def setData(self, index, value, role):
        if (role == QtCore.Qt.EditRole):
            if self.coordDB.column_names[index.column()] in ["SCORE", "COMMENT"]:
                ...
                QtGui.QMessageBox.information(self, "Read only", "This should not happen.", "OK")
                return False
            return True
...
class CoordDBView(QtGui.QTableView):
    def __init__(self): 
    ...
    def select_rows(self, rows):
        ...
        if modifiers == QtCore.Qt.ControlModifier:
            flags =  QtGui.QItemSelectionModel.Rows |QtGui.QItemSelectionModel.Toggle
        else:
            flags  = QtGui.QItemSelectionModel.Rows | QtGui.QItemSelectionModel.ClearAndSelect
        r = [self.model().mapFromSource(x) for x in source_indices]
        s = QtGui.QItemSelection()
        for rr in r:
            s.merge(QtGui.QItemSelection(rr,rr), QtGui.QItemSelectionModel.Rows | QtGui.QItemSelectionModel.Select)
    ...
    def save_xyz(self, fname):
        ...
        if len(h) > 0 or len(hc) > 0:
         r = QtGui.QMessageBox.question(self, "Margo GUI", "A row or column filter is active. Do you want to save all entries", "Save all", "Only filtered")
        if (r != 0):
    ...
    def save_shp(self, fname):
        try:
            ...
            QtGui.QMessageBox.information(self, "Margo GUI", "Please load a valid projection file.")
            return
        h = self.model().hide_row_indices
    ...
    def rowCountChanged(self, new, old):
        QtGui.QTableView.rowCountChanged(self, new, old)
        self.rcChange.emit()

    def set_hidden_columns(self, hidden):
       # print "hiding columns"
        for i in range(self.model().columnCount()):
            name = str(self.model().headerData(i, QtCore.Qt.Horizontal, QtCore.Qt.DisplayRole).toString())
            #print name
...
class MainWindowContents(QtGui.QWidget):
    ...
    def select_record(self, ID, caller):
        ...
        #self.table.selectionModel().select(new_index, QtGui.QItemSelectionModel.Toggle | QtGui.QItemSelectionModel.Rows)
    ...
    def insert_missing_point(self, x,y):
        if len(self.table.model().hide_row_indices) != 0:
            QtGui.QMessageBox.information(self, "Margo GUI", "Please clear the row filter first.")
            return
        else:
    ...
    def initUI(self):
        ...
        self.infobox = QtGui.QPlainTextEdit() #QtGui.QLabel()
        ...
        self.filter_box = QtGui.QTextEdit()
        self.figure = plt.figure()
        self.canvas = FigureCanvas(self.figure)
        
        self.filter_button = QtGui.QPushButton("Filter")
        self.unfilter_button = QtGui.QPushButton("Clear filter")
        self.selected_filter_button = QtGui.QPushButton("Selection")
        hgroup = QtGui.QGroupBox()
        ...
        self.filter_tab = QtGui.QGroupBox()
...
#        update_profile()
    ...
    def update_map2(self):
        ...
        #print(s)
        self.the_map.page().mainFrame().evaluateJavaScript(s);
    ...
    def map_show_ids(self, ids, cr = None, zoom = None): 
        ...
        if zoom == None:
            S = self.the_map.page().mainFrame().evaluateJavaScript("map.getZoom()").toInt();
            if S[1]:
                zoom = S[0]
            else:
                zoom = 13
        ...
        if len(ids) > self.max_points_gmap:
            r = QtGui.QMessageBox.question(self, "Margo GUI", "The number of points ("+str(len(ids)) +") is large. Show only a random selection of "+ str(self.max_points_gmap)+"? Later you can zoom in to a smaller area and use 'Update map'.", "Truncate", "Show all points")
            if (r == 0):
    ...
    def show_map(self, map_all = False): 
        ...
        if True:
            ...
            try:
                ...
                S = self.the_map.page().mainFrame().evaluateJavaScript(q);
...
class MainWindow(QtGui.QMainWindow):
    ...
    def createDockWidgets(self, widgets, titles):
        self.docks = []
        self.toolsMenu = QtGui.QMenu("&Tools")
        for i in range(len(widgets)):
            d = QtGui.QDockWidget(titles[i], self)
    ...
    def suggest_open(self, filename):
        ...
        r = QtGui.QMessageBox.question(self, "Margo GUI", "Processing finished. Do you want to open the last result?", QtGui.QMessageBox.Yes | QtGui.QMessageBox.No)
    ...
    def showRasterDialog(self):
        name = str(QtGui.QFileDialog.getOpenFileName(self, 'Open file', self.raster_filename))
        if name == "":
            return
        try:
            ...
            QtGui.QMessageBox.information(self, "Error", "Could not load raster file or projection.")
   
        self.mwc.update_raster()

    def showProjectionDialog(self):
        name = str(QtGui.QFileDialog.getOpenFileName(self, 'Open file', config.get("projection_filename"), "*.prj"))
        if name == "":
            return
    ...
    def showDialog(self):
        name= str(QtGui.QFileDialog.getOpenFileName(self, 'Open file', self.current_fname, "xyz, shp (*.xyz *.shp)"))
        if name == "":
            return
        self.load_file(name)
    ...
    def showMultiDialog(self):
        names= QtGui.QFileDialog.getOpenFileNames(self, 'Select files to merge', self.current_fname, "xyz, shp (*.xyz *.shp)")
    ...
    def load_file(self, names):
        ...
        self.mwc.table.setSelectionBehavior(QtGui.QAbstractItemView.SelectRows)
    ...
    def showSaveDialog(self):
        name = str(QtGui.QFileDialog.getSaveFileName(self, 'Save file', self.current_fname))
    ...
    def showImportJoinDialog(self):
        ...
        name = QtGui.QFileDialog.getOpenFileName(self, 'Import and join file', path, "xyz, shp (*.xyz *.shp)")
        if name is None or name == "": return
        name = str(name)
    ...
    def showExportCSVDialog(self):
        ...
        csvname = QtGui.QFileDialog.getSaveFileName(self, 'Export file', path, "*.csv")
    ...
    def showExportSHPDialog(self):
        ...
        name = QtGui.QFileDialog.getSaveFileName(self, 'Export file', path, "*.shp")
        if name is None or name == "": return
