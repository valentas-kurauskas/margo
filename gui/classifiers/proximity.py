...
class ProximityClassifier(KnnLinClassifier):
    ...
    Parameters: x_knn = data("RAW_HEIGHTS"), y = exceeds("SCORE", 0.5), k=10, distance = 40, result = "LR_PROB"
    '''
    def __init__(self, x_knn = data("RAW_HEIGHTS"), y = exceeds("SCORE", 0.5), k=10, distance = 40, result="LR_PROB"):
        KnnLinClassifier.__init__(self, x=data(1, "KNN_PROB", "KNN_PROB_NN"), x_knn=x_knn, y=y, k=k,  preprocess_fun = lambda db: nn_update(db, "KNN_PROB", "KNN_PROB_NN", distance,False),result=result)
